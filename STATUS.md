...
overridden with `CITY`, `LAT`, `LON`, and `TIMEZONE` environment variables. Recipients come from the gitignored `recipients.txt` locally or the `RECIPIENT_EMAILS` secret in CI, with `RECEIVER_EMAIL` as a fallback.
